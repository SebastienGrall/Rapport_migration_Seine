...
plot_debit <- plot_grid(
  debit_seine_histo,
  debit_oise_histo,
  debit_aisne_histo,
  nrow = 3,                             # empiler les deux lignes
  rel_heights = c(1, 1,1)                 # tu peux ajuster la hauteur relative (ex: c(1.2, 1))
)

source_label <- ggdraw() +
  draw_label("Source des données : HydroPortail",
             x = 0.5, hjust = 0.5, size = 10, fontface = "italic")

debit_seine_oise_aisne <- plot_grid(
  plot_debit,    # Graphiques au centre
  source_label, # Source en bas
  ncol = 1,
  rel_heights = c(1, 0.08) # Ajuste la place relative des éléments
)
