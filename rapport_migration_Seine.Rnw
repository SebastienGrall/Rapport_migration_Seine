...
filecolor=blue, 
urlcolor=blue, 
pdftitle= {Suivi des poissons migrateurs sur les stations de contrôle des migrations (STACOMI) du bassin de la Seine\\ Année \Sexpr{annee} }, pdfauthor={Sébastien Grall},
pdfsubject={stacomi},
...
\fancyhf{}
\rhead{\rightmark}
\lfoot{\footnotesize{Suivi des poissons migrateurs sur les stations de contrôle des migrations (STACOMI) du bassin de la Seine, année \Sexpr{annee}}} %Insere un pied de page avec le texte a gauche.
\rfoot{\thepage}
...
debit_seine_histo<-ggplot() +
  ...
  labs(x = "", y = bquote(bold('débits de la Seine ('* ~ m^3/s*')')),caption = "Source des données : HydroPortail") +
  theme_bw()+
    theme(
...
png(file=str_c(imgwd,"debit_seine_histo.png"),width=14,height=6,unit="in",res=200)
print(debit_seine_histo)
dev.off()
...
debit_oise_histo<-ggplot() +
  ...
  labs(x = "", y = bquote(bold('débits ('* ~ m^3/s*')')),caption = "Source des données : HydroPortail") +
  theme_bw()+
    theme(
    axis.text = element_text(size = 15),
    axis.title.y = element_text(size = 15),
...
png(file=str_c(imgwd,"debit_oise_histo.png"),width=14,height=6,unit="in",res=200)
print(debit_oise_histo)
dev.off()
...
png(file=str_c(imgwd,"debit_aisne_histo.png"),width=14,height=6,unit="in",res=200)
print(debit_aisne_histo)
dev.off()
...
\title{Suivi des poissons migrateurs sur les stations de contrôle des migrations du bassin de la Seine,
année \Sexpr{annee}}
   ...
   \centering
    ...
    \colorbox{bleuTitre}{
        \parbox{\textwidth}{
            \centering
            \vspace{0.5cm}
            {\Huge \textbf{\textcolor{white}{Suivi des poissons migrateurs sur les stations de contrôle des migrations du bassin de la Seine}}}\par
            \vspace{0.3cm}
    ...
    \colorbox{grisBloc}{
        \parbox{0.9\textwidth}{
            ...
            Association Loi 1901 déclarée le 2 janvier 2007 à la Préfecture de la Seine-Maritime\par
            Agréée sur la région Normandie en qualité d’association de protection de l’environnement\par
            par arrêté préfectoral du 20 décembre 2022
            \vspace{0.5cm}
        }
    }
    \vspace{1.5cm}
...
\caption{Régime hydrologique de la Seine à Vernon, la moyenne interannuelle des dix années précédentes (\Sexpr{annee-11} à \Sexpr{annee-1}) est représentée en bleu, les débits journaliers de l'année sont présentés en noir}
