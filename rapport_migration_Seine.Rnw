...
##graph debit histo Seine


r_env<-new("report_env")
r_env<-choice_c(r_env,
                stationMesure=c("Poses_debit"),
                datedebut=str_c(annee-11,"-01-01"),
                datefin=str_c(annee,"-12-31"),
                silent=FALSE)	
r_env<-connect(r_env)
debit<-r_env@data
debit$year<-strftime(debit$env_date_debut,format="%Y")
debit$mois<-strftime(debit$env_date_debut,format="%m")

debit_inter<-subset(debit,year<annee)
debit_annee<-subset(debit,year==annee)

debit_inter<-summarise(group_by(debit_inter,mois),debit=mean(env_valeur_quantitatif)) %>%
      as.data.frame()
debit_inter$mois<-as.POSIXct(str_c(annee,"-",debit_inter$mois,"-01"),tz="GMT")


debit_seine_histo<-ggplot()+geom_area(data=debit_inter,aes(x=mois,y=debit), fill="#318CE7")+
  geom_line(data=debit_annee,aes(x=env_date_debut,y=env_valeur_quantitatif),colour="black",linewidth=1)+theme_bw()+
  scale_x_datetime(date_labels = "%B", date_breaks = "month",limits = c(min(debit_inter$mois),
          max(debit_inter$mois)),expand = c(0,0))+xlab("")+ylab(bquote(bold('débits ('* ~ m^3/s*')')))+
  theme(axis.text = element_text(size=15),axis.title = element_text(size=15,face="bold"),plot.margin = unit(c(1,1.3,1,1), "cm"))+
  scale_y_continuous(expand = expansion(mult = c(0,0.1)))
png(file=str_c(imgwd,"debit_seine_histo.png"),width=42,height=20,unit="cm",res=200)
