...
data_long$bjo_jour_2024 <- as.Date(
  paste0("2024-", format(data_long$bjo_jour, "%m-%d"))
)

plot_ridgeline_ala<-ggplot(data_long,
       aes(x = bjo_jour_2024, y = Saison, fill = station)) +
...
data_taille_alose <- data_taille_alose %>% mutate(station = case_when(
                            ope_dic_identifiant %in% c(101,103,105)  ~ "Poses",
                            ope_dic_identifiant == 108  ~ "Pontoise"))

ggplot(data_taille_alose)+geom_boxplot(aes(y=car_valeur_quantitatif,x=mois))+
  facet_grid(rows=vars(station),col=vars(annee))
